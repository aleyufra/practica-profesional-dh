...
socialNetworkService.create = async (object) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			return reject(
				"Error al crear las redes sociales: " + error.message
			);
		}
	});
};
...
socialNetworkService.update = async (object) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			return reject(
				"Error al actualizar las redes sociales: " + error.message
			);
		}
	});
};
...
socialNetworkService.delete = async (object) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			await db.SocialNetwork.destroy({
				where: { candidate_id: candidate_id },
			});
			...
			// en caso de error retornamos como promesa rechazada
			return reject(
				"Error al borrar las redes sociales: " + error.message
			);
		}
	});
};
 ...
 * @returns {Object}
 */
socialNetworkService.whereConditions = (conditions, queries) => {
	const { linkedin } = queries;
	...
	if (linkedin) {
		conditions[Op.or].push({
			"$SocialNetwork.linkedin$": { [Op.like]: `%${linkedin}%` },
		});
	}
};
